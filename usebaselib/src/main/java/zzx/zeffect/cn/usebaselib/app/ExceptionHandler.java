...
import zzx.zeffect.cn.usebaselib.async.DoAsync;
import zzx.zeffect.cn.usebaselib.file.FileIOandOperation;
...
//                Toast.makeText(context, "程序出现错误,即将退出！", Toast.LENGTH_LONG).show();
    ...
    private void saveExceptionToFile(Throwable ex) {
        ...
        sb.append(result);
        new DoAsync<String, Void, Void>(context) {
            @Override
            protected Void doInBackground(Context pTarget, String... voids) throws Exception {
                File saveFile = new File(pTarget.getExternalFilesDir("Log"), "error_" + getCurrentDate() + ".txt");
                FileIOandOperation.writeFile(saveFile, voids[0]);
                return null;
            }
        }.execute(sb.toString());
